# backend/services/csv_import.py - UPDATED with geocoding
"""
CSV Import Service with automatic geocoding for new properties.
"""
...
class CSVImportService:
    ...
    Now includes automatic geocoding for new properties.
    
    Pass 1: Import all shopping centers
    Pass 2: Geocode new shopping centers
    Pass 3: Import all tenants (with valid FK references)
    """
    ...
    def __init__(self, enable_geocoding=True):
        ...
        self.geocoding_service = GeocodingService() if enable_geocoding else None
    
    def import_csv(self, csv_file_path: str) -> Dict:
        """
        Main import method using three-pass approach.
        ...
        shopping_centers, new_centers = self._import_shopping_centers(csv_file_path)
        
        # Pass 2: Geocode new shopping centers
        geocoded = 0
        if self.enable_geocoding and new_centers:
            print(f"\n=== Pass 2: Geocoding {len(new_centers)} New Properties ===")
            geocoded = self._geocode_shopping_centers(new_centers)
        
        # Pass 3: Import tenants
        print("\n=== Pass 3: Importing Tenants ===")
        tenants_created = self._import_tenants(csv_file_path, shopping_centers)
        
        # Summary
        result = {
            'shopping_centers_created': len(new_centers),
            'shopping_centers_existing': len(shopping_centers) - len(new_centers),
            'properties_geocoded': geocoded,
        ...
        print(f"\n=== Import Complete ===")
        print(f"Shopping Centers Created: {result['shopping_centers_created']}")
        print(f"Shopping Centers Existing: {result['shopping_centers_existing']}")
        print(f"Properties Geocoded: {result['properties_geocoded']}")
    ...
    def _import_shopping_centers(self, csv_file_path: str) -> tuple:
        """
        Pass 1: Create all shopping centers and return mappings.
        
        Returns:
            Tuple of (all_centers dict, list of newly created centers)
        """
        shopping_centers = {}
        new_centers = []
        row_num = 0
        ...
        return shopping_centers, new_centers
    
    def _geocode_shopping_centers(self, new_centers: List[ShoppingCenter]) -> int:
        """
        Pass 2: Geocode newly created shopping centers.
        
        Args:
            new_centers: List of newly created ShoppingCenter instances
            
        Returns:
            Number of properties successfully geocoded
        """
        if not self.geocoding_service or not self.geocoding_service.api_key:
            print("  Warning: Geocoding skipped (API key not configured)")
            return 0
        
        geocoded_count = 0
        
        for i, shopping_center in enumerate(new_centers, 1):
            address = shopping_center.get_full_address()
            
            if not address:
                print(f"  [{i}/{len(new_centers)}] {shopping_center.shopping_center_name}: No address, skipping")
                continue
            
            print(f"  [{i}/{len(new_centers)}] Geocoding {shopping_center.shopping_center_name}...", end='')
            
            success = self.geocoding_service.geocode_shopping_center(shopping_center)
            
            if success:
                print(f" ✓ ({shopping_center.latitude}, {shopping_center.longitude})")
                geocoded_count += 1
            else:
                print(f" ✗ Failed")
                self.warnings.append(f"Failed to geocode: {shopping_center.shopping_center_name}")
            
            # Rate limiting - wait between requests
            if i < len(new_centers):
    ...
    def _import_tenants(self, csv_file_path: str, shopping_centers: Dict[str, ShoppingCenter]) -> int:
        """
        Pass 3: Create all tenants linked to shopping centers.
        ...
        try:
            with open(csv_file_path, 'r', encoding='utf-8') as file:
                ...
                for row in reader:
                    ...
                    try:
                        # Create tenant with committed shopping center FK
    ...
    # Parsing utility methods (unchanged from original)
...
# Helper function for management commands
def run_import(csv_path: str, enable_geocoding=True):
    """
    Helper function to run import from management command.
    
    Args:
        csv_path: Path to CSV file
        enable_geocoding: Whether to geocode new properties (default: True)
    
    Usage:
        from services.csv_import import run_import
        run_import('/path/to/masterdatabase.csv')
    """
    service = CSVImportService(enable_geocoding=enable_geocoding)
